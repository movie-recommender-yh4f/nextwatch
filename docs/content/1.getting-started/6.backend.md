...
| `/api/auth/email-exists` | `POST` | None | Validate a candidate email address and return whether it already exists |
| `/api/onboarding/status` | `GET` | Bearer token | Return whether the signed-in user has completed onboarding |
| `/api/onboarding/complete` | `POST` | Bearer token | Seed watched movies and mark onboarding complete |
...
| `/api/movies/metadata` | `POST` | Bearer token | Hydrate a list of TMDB IDs from cached Supabase movie rows |
