<template>
  <div class="p-6 h-full flex flex-col overflow-hidden bg-surface relative">
    <div v-if="user" class="flex flex-col items-center w-full min-h-full overflow-y-auto pb-20">
      <UserProfileHeader />
      <NuxtLink
        to="/mylist"
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="w-10 h-10 rounded-full bg-rose-100 dark:bg-rose-900/30 flex items-center justify-center text-rose-500"
          >
          ...
          <div class="flex flex-col">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400"
              >{{ myList.length }} movies to watch</span
            >
          </div>
        </div>
    ...
    <Transition name="fade">
      <div
        v-if="undoAction"
        class="fixed left-1/2 top-6 z-50 flex max-w-sm -translate-x-1/2 items-center gap-3 rounded-full border border-gray-200 bg-white px-5 py-3 text-gray-800 shadow-lg dark:border-transparent dark:bg-gray-700 dark:text-white"
      >
